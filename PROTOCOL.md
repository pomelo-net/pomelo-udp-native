...
# pomelo 1.03 - Modified version from netcode 1.02
...
Prior to encryption the private connect token data has the following binary format.
    ...
    [client id] (int64) // globally unique identifier for an authenticated client
    [timeout seconds] (int32) // timeout in seconds. negative values disable timeout (dev only)
...
The first packet type _connection request packet_ (0) is not encrypted and has the following format:
    ...
    [version info] (13 bytes)       // "NETCODE 1.03" ASCII with null terminator.
    [protocol id] (8 bytes)
...
Prior to encryption, packet types >= 1 have the following format:
    ...
    [per-packet type encrypted data] (variable length according to packet type)
    [per-packet type unencrypted data] (variable length according to packet type)
...
After the sequence number comes the per-packet type encrypted data:

_connection denied packet_ (0 bytes):

    <no data>

_connection challenge packet_ (308 bytes):

    [challenge token sequence] (uint64)
    [encrypted challenge token data] (300 bytes)
    
_connection response packet_ (308 bytes):

    [challenge token sequence] (uint64)
    [encrypted challenge token data] (300 bytes)

_connection ping packet_ (3 to 19 bytes):
...
_connection pong packet_ (4 to 25 bytes):

    [pong encrypted meta] (uint8)
    [ping sequence] (1 to 2 bytes)
    [ping recv time] (1 to 8 bytes)
    [pong delta time] (1 to 8 bytes)

_connection payload packet_ (1 to 1200 bytes):

    [payload data] (1 to 1200 bytes)
    
_connection disconnect packet_ (0 bytes):
...
* If the per-packet type data size does not match the expected size for the packet type, ignore the packet.
    ...
    * [3,17] bytes for _connection ping packet_
    * [2,9] bytes for _connection pong packet_
...
After receiving ping packet, the receiver will reply with pong packet associated with the ping sequence in the received ping packet. Maximum value for ping sequence is 0xFFFF (maximum 2 bytes)
...
When the client receives a _connection ping packet_ from the server, it stores the client index and max clients in the packet, and transitions to _connected_.
...
If a _connection request denied_ packet is received while in _sending challenge response_ the client transitions to _connection denied_. If neither a _connection ping packet_ or a _connection denied packet_ are received within the timeout period specified in the connect token, the client transitions to _challenge response timed out_.
...
While _connected_ the client application may send _connection payload packets_ to the server. In the absence of _connection payload packets_ sent by the client application, the client generates and sends _connection ping packets_ to the server at some rate, like 10HZ.

If no _connection payload packet_ or _connection ping packet_ are received from the server within the timeout period specified in the connect token, the client transitions to _connection timed out_. 
...
* Respond with a _connection ping_ packet.
...
In the absence of _connection payload packets_ sent to a client, the server generates and sends _connection ping packets_ to that client at some rate, like 10HZ.

While the _confirmed_ flag for a client slot is false, each _connection payload packet_ sent to that client has a _connection ping packet_ sent before it. This communicates the _client index_ and the _max clients_ to that client, which it needs to transition to a fully connected state.

When the server receives a _connection payload packet_ or a _connection ping packet_ from an unconfirmed client, it sets the _confirmed_ flag for that client slot to true, and stops prefixing _connection payload packets_ with _connection ping packets_.
...
If no _connection payload packet_ or _connection ping packet_ are received from a client within the timeout period specified in the connect token, or the server receives a _connection disconnect_ packet from a client, the client slot is reset and becomes available for other clients to connect to.
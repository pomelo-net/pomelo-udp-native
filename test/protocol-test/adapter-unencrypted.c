...
uint8_t pomelo_adapter_get_capability(pomelo_adapter_t * adapter) {
    ...
    return POMELO_ADAPTER_CAPABILITY_UNENCRYPTED;
}
...
void pomelo_adapter_recv_callback(
    ...
    pomelo_adapter_on_recv(adapter, address, buffer_vector, status, false);
}
...
int pomelo_adapter_send(
    ...
    assert(packet != NULL);
    if (encrypted) {
        return -1; // Encrypted packets are not supported.
    }

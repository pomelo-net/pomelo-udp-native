...
void pomelo_protocol_clock_init(
    ...
    memset(clock, 0, sizeof(pomelo_protocol_clock_t));
...
void pomelo_protocol_clock_set(
    pomelo_protocol_clock_t * clock,
    uint64_t value
) {
    assert(clock != NULL);
...
bool pomelo_protocol_clock_sync(
    ...
    switch (clock->level) {
        case POMELO_PROTOCOL_CLOCK_SYNC_LEVEL_HIGH:
            return pomelo_protocol_time_sync_high(clock, rtt_var, offset);

        case POMELO_PROTOCOL_CLOCK_SYNC_LEVEL_MEDIUM:
            return pomelo_protocol_time_sync_medium(clock, rtt_var, offset);

        case POMELO_PROTOCOL_CLOCK_SYNC_LEVEL_LOW:
            return pomelo_protocol_time_sync_low(clock, rtt_var, offset);
...
bool pomelo_protocol_time_sync_high(
    ...
    if (clock->high_sync_count < POMELO_TIME_HIGH_MIN_TIMES_OF_PING) {
        ...
        clock->level = POMELO_PROTOCOL_CLOCK_SYNC_LEVEL_MEDIUM;
    }
...
bool pomelo_protocol_time_sync_medium(
    ...
    if (variance < POMELO_TIME_MEDIUM_THRESHOLD_RECENT_OFFSETS_VAR) {
        clock->level = POMELO_PROTOCOL_CLOCK_SYNC_LEVEL_LOW;
    }
